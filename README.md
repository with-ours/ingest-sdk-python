...
The Ours Privacy Python library provides convenient access to the Ours Privacy REST API from any Python 3.9+
...
```python
import asyncio
from ours_privacy import DefaultAioHttpClient
from ours_privacy import AsyncOursPrivacy
...
print(response.default_properties)
```

## Handling errors

When the library is unable to connect to the API (for example, due to network connection problems or a timeout), a subclass of `ours_privacy.APIConnectionError` is raised.

When the API returns a non-success status code (that is, 4xx or 5xx
response), a subclass of `ours_privacy.APIStatusError` is raised, containing `status_code` and `response` properties.

All errors inherit from `ours_privacy.APIError`.
...
except ours_privacy.APIConnectionError as e:
    print("The server could not be reached")
    print(e.__cause__)  # an underlying Exception, likely raised within httpx.
except ours_privacy.RateLimitError as e:
    print("A 429 status code was received; we should back off a bit.")
except ours_privacy.APIStatusError as e:
...
You can enable logging by setting the environment variable `OURS_PRIVACY_LOG` to `info`.
...
```python
import httpx
from ours_privacy import OursPrivacy, DefaultHttpxClient

client = OursPrivacy(
    # Or use the `OURS_PRIVACY_BASE_URL` env var
...
```py
import ours_privacy
print(ours_privacy.__version__)
```
